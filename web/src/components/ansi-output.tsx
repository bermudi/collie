...
// The card that stands in for one image placeholder cluster. `url === null` is the "[Image]"
// badge: a placeholder the journal could not match, or a journal read that has not answered yet,
// still has to say "a picture is here", which is the whole difference from the black box this
// replaces.
//
// The card is an ANCHOR to the blob, so it is keyboard reachable and long-pressable, and the href
// is a URL `imageSrc` already vetted (`lib/api.ts`) — a blob path this bridge serves, or inline
  ...
  url === null ? (
    ...
    >
      <a
        ...
        title="matched by order, open History to check"
...
export const AnsiOutput = memo(function AnsiOutput({
  ...
  // A blob read can answer 404 (the file was pruned) or fail mid-transfer. Either way the operator
  // must not be shown a broken-image glyph, so the URL that failed is remembered and the cluster
  // falls back to the "[Image]" badge it would have had with no image at all.
  ...
  // Cluster images are indexed across the WHOLE screen, not per block.
  let clusterIndex = 0;
