...
export interface AnsiOutputProps {
  ...
  /** Pre-built blocks, so a caller that already ran the grammar hands the result over (the lifted
   * cards dock — see `lib/harness` — builds them once for the mirror and the card). */
...
// ── COLLAPSED BLANK RUNS ────────────────────────────────────────────────────────────────────────
// An agent that paints a terminal image with a direct Kitty placement reserves its rows as
// whitespace in the grid — the picture sits in the terminal's graphics layer, never in the text,
// so the pane read hands the mirror a wall of blank lines and nothing to detect (that is why the
// placeholder-cluster machinery was removed: it only ever fired for harnesses that print unicode
// placeholders, like omp — see .adr/0052). A run that tall is dead space the reader scrolls
// through for no reason, so runs of BLANK_RUN_MIN or more collapse into one labelled line.
//
// "Blank" is whitespace-only text with no painted background. A run of cells carrying a bg fill is
// a band of colour — a selected region, a status block — which is content, not space.
const BLANK_RUN_MIN = 4;
...
/** Index of the last line of the blank run starting at `from`, or `from - 1` when there is none. */
function blankRunEnd(lines: readonly StyledLine[], from: number): number {
  let end = from;
...
export const AnsiOutput = memo(function AnsiOutput({
  ...
  // The mirror is agent output and is not translated — but the one string the collapsed-blank-run
  // label renders is Collie's own words, so this subscribes for the same reason every t() caller does.
  useLocale();
  ...
  const renderBlock = (block: RawBlock, bi: number) => {
    ...
    for (let li = 0; li < block.lines.length; ) {
      const blankEnd = blankRunEnd(block.lines, li);
      if (blankEnd - li + 1 >= BLANK_RUN_MIN) {
        // Collapsed, not deleted: the whitespace still EXISTS in the find/link coordinate space,
        // so the shared offset walks every skipped line. Dropping that walk was the image-cluster
        // version of this bug — every match below the run landed a screenful early.
        for (let k = li; k <= blankEnd; k++) {
          if (k > 0) offset += 1; // the "\n" separating this line from the previous
          offset += lineText(block.lines[k]!).length;
        }
        nodes.push(
          <Fragment key={`blank:${bi}:${li}`}>
            {li > 0 ? "\n" : null}
            <span className="text-muted-foreground/60">
              {t("mirror.blankLines", { n: blankEnd - li + 1 })}
            </span>
          </Fragment>,
        );
        li = blankEnd + 1;
