...
describe("BottomSheet: pull-driven peek", () => {
  ...
  it("caps the panel at the content column while the backdrop stays the whole screen", () => {
    // The sheet is the app's only floating layer and it holds ordinary rows, so it stops at the
    // same 640px every route body uses. Uncapped, the panel spanned the whole viewport — 1366px
    // on a landscape 13-inch iPad — for rows that were drawn for a phone. The dim is the
    // exception and must not be capped with it.
    const { container } = render(
      <BottomSheet open onClose={vi.fn()} title="Switch pane">
        body
      </BottomSheet>,
