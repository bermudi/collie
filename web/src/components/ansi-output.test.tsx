...
// A direct Kitty placement reserves its rows as whitespace in the grid — the picture lives in the
// terminal's graphics layer and never reaches the pane read, which is exactly the black box the
// removed placeholder machinery could not fix for it (.adr/0052). What the mirror does with that
// wall of nothing is collapse it. These pin the shape of that collapse: the threshold, the
// painted-band exception, and — the one that bit the image cards before — every character the
// collapsed rows held still counts in the find/link coordinate space.
describe("collapsed blank runs", () => {
  it("collapses a tall blank run into one labelled line", () => {
    const text = ["header", ...Array<string>(9).fill(""), "footer"].join("\n");
    const { container } = render(<AnsiOutput text={text} />);
    expect(container.textContent).toContain(en["mirror.blankLines"].replace("{n}", "9"));
    expect(container.textContent).toContain("footer");
    // Nine rows became one — the pre's raw text carries no run of consecutive blank lines.
    expect(container.querySelector("pre")!.textContent).not.toContain("\n\n\n");
  });

  it("leaves ordinary paragraph spacing alone", () => {
    const { container } = render(<AnsiOutput text={`header\n\n\nfooter`} />);
    expect(container.textContent).toContain("\n\n\n");
    expect(container.textContent).not.toContain("blank lines]");
  });

  it("does not collapse a run of painted lines — a coloured band is content, not space", () => {
    const band = `${ESC}[41m     ${ESC}[0m`;
    const text = `header\n${band}\n${band}\n${band}\n${band}\n${band}\nfooter`;
    const { container } = render(<AnsiOutput text={text} />);
    expect(container.textContent).not.toContain("blank lines]");
  });

  it("advances the find offset over the collapsed rows", () => {
    // The same bug the image cluster had: skipped lines must still walk the shared offset, or a
    // match below the run is addressed a screenful early and highlights the wrong characters.
    const text = `aaa\n\n\n\n\n\n\nbbbbbbb needle ccc`;
