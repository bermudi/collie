...
// (upstream e12b4334: ~275px → 119px at 390px wide, every key still ≥36px tall)
...
// Two views became one pad: the old Keys/123 segmented toggle and stacked rows collapsed into the
// 7-column grid below, with the digits behind a chip (upstream e12b4334). The armed modifier, the
// key queue, and the open panel persist across chip switches so a composed sequence survives
// opening the digit grid mid-chord.
...
export function NavTray({ onSend, presets = CONTROL_PRESETS, onQueueChange, disabled }: NavTrayProps) {
  ...
  return (
    <div className="space-y-0.5 border-t border-rule bg-muted/30 px-2 py-1.5">
      {/* Staging strip — visible only while composing (a modifier armed or keys queued); the
          review-and-Send surface replaces the old "⇧ armed" hint line. */}
      <KeyQueueStrip
        queue={queue}
        mods={activeMods}
        onRemove={removeAt}
      ...
      {/* Row 1: Esc, Tab, the three modifiers, Up, Enter. Row 2: a quick Ctrl+C, Space spanning the
          ...
          pad; the seven-column row shape and the Tab/Space placement are new. Enter and Shift draw
          glyphs (⏎ / ⇧) with the full word as each button's aria-label, so a 390px row never clips
          a label a reader is addressing by name. */}
      ...
      {open === "123" && (
        /* Pick a numbered option — the digit grid rides the ordinary fire() path, so an armed
           modifier / a queue built on the pad still applies to a digit press. */
        <div className="grid grid-cols-5 gap-1">{DIGITS.map((d) => navBtn(d, [d]))}</div>
      )}

      {open === "presets" && (
        /* The labelled Ctrl chords. On the immediate path a danger preset needs a second tap; while
           composing a tap just stages its chords for review. An operator's `keys.toml` rows arrive
           here as the same CtrlDef list, so a multi-chord row sends as one batch and an armed
           modifier stages it — no special-casing. */
        <div className="grid grid-cols-3 gap-1">
          {presets.map((item) => {
            ...
            return (
              ...
              >
                {isPending ? "Confirm?" : phase === "done" ? <Check className="size-4" /> : item.label}
              </Button>
            );
          })}
        </div>
      ...
      {open === "fkeys" && (
        /* Function keys (#119), on the ordinary navBtn path: the press echo, key-queue staging, and
           chords with armed modifiers (ctrl+F7, …) all come for free — no special-casing. */
