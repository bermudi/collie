...
// nothing here decides what the buttons are. (upstream fc8d1be9)
//
...
//
// THE BLACK-BRANDED FALLBACK IS THE ONE CASE THAT NEEDS A HAIRLINE. A brand with no legible accent
// (Codex, pi — both officially black) falls back to the app's muted ground; see `accent` in
// agent-icon-data.ts for why absent is a real answer there. On the belt's own ground that fallback
// stops separating in dark, so the muted section, and only it, colours BELT_SECTION's reserved left
// edge with `border-l-border`.
//
...
// The tint alone says "not Collie's"; the mark says WHOSE, at a glance and before a word is read.
//
// IT ADDS NO REFUSAL OF ITS OWN. `disabled` (the composer's `locked`) greys every button in place.
// Everything else is refused inside `send()`: a dialog on screen is refused there with the existing
// status line, and a WORKING pane is not refused at all, because the Agent palette does not refuse
// one either. One gate, one place. The checkmark is the honest signal — it appears only when
// `send()` resolved true, so a tap that was refused shows nothing and the label stays put.
//
// It does not collapse or animate. A section that appeared and disappeared would move the input
// under the thumb, and the no-shift rule says reserve, never reflow — so its presence is decided by
// the pane's agent, which does not change while the pane is on screen.
 ...
 * no React import, no DOM — and a lucide component is both.
 *
...
export function HarnessBar({ agent, mine, onRun, disabled }: HarnessBarProps) {
  ...
  return (
    ...
    >
      {/* The mark, not a button: `aria-hidden` drops AgentIcon's own `role="img"` and its label
          out of the tree, so a reader hears the group's name once and not a logo before every
          command. It carries no tap area either — it is the section's tag. */}
      ...
      {items.map((item) => {
        ...
        return (
          <Button
            ...
            disabled={disabled}
            onClick={() => fire(item)}
            aria-label={armed ? `Tap again to run ${item.command}` : item.label}
            className={cn(
...
function accentFor(agent: string | undefined | null): string | undefined {
  ...
  return AGENT_BRANDS[canonicalAgent(agent.toLowerCase().trim())]?.accent;
}
