...
export const Composer = forwardRef<ComposerHandle, ComposerProps>(function Composer(
  ...
  // What this host takes as an attachment, off the same one-shot /api/config read. On a bridge
  // that publishes nothing (older than the field, or the read has not landed) `uploadLimits`
  // answers with the contract that shipped before attachments — images, 10 MB — so the button is
    ...
    <>
      <div className="border-t border-rule bg-chrome px-3 pb-[calc(env(safe-area-inset-bottom)_+_0.5rem)] pt-0">
        ...
        {/* gap-3, not gap-2: with the attach button moved inside the field this row is only the
            ...
            field reaches 4px past the field's own border on every side, measured. The belt above
            already carries `mb-1` for its own reason (actions-row.tsx), but that 4px sits OUTSIDE
            this row and the ring bleeds straight through it — without this padding the ring's top
            edge lands exactly on the belt's bottom edge, touching. `pt-1` absorbs the ring's 4px
            reach INSIDE the row instead, so the belt's existing `mb-1` is what remains as
            clearance. The bottom needs no match: the chrome container's own padding below this row
            is already 8px, twice the ring's 4px reach. Adding `pb-1` would only push that to 8px
