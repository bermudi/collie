...
export function PaneActionsSheet({
  ...
  // Asked of the PANE's own machine (M22/03): a member runs
  ...
  // Every pane this build fronts lives on that machine, so "focus" always runs on the mux this page
  // already knows the name of. The empty-string fallback below stays for the "bridge hasn't answered
  // yet" case `useMuxName()` itself returns — both get the same generic, never-wrong copy.
  const focusMux = localMuxName;
