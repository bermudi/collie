...
// on-demand route the History view uses. A count that shrinks (the terminal scrolled the image off)
// asks nothing: the images in hand still cover the clusters that are left, and the alignment runs
// from the end.
//
...
export function useMirrorImages({
  ...
  const fetchImages = useCallback(async (): Promise<void> => {
    ...
    inFlightAbort.current = abort;
    try {
      const page = await fetchHistory(paneId, { limit: TURNS }, session, abort.signal);
  ...
  // Images belong to the pane they were read from. Keyed on the ADDRESS: the same pane id in
  // another session is a different terminal.
