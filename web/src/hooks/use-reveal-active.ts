...
//
// A manual scroll never triggers a reveal: the effect re-runs only on `activeKey` change (and
// mount), never on scroll events, so the operator's scroll position is only ever touched at the
