...
describe("useDisplayPrefs", () => {
  ...
  it("returns defaults when localStorage is empty", () => {
    ...
    expect(result.current.prefs).toEqual({ wrap: true, fontSize: 10, rawTerminal: false, tapToFocus: true, expandClippedReply: true });
  });
  ...
  it("loads persisted prefs from localStorage on mount", () => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ wrap: false, fontSize: 14, rawTerminal: true, tapToFocus: false, expandClippedReply: true }));
    const { result } = renderHook(() => useDisplayPrefs());
    expect(result.current.prefs).toEqual({ wrap: false, fontSize: 14, rawTerminal: true, tapToFocus: false, expandClippedReply: true });
  });
  ...
  it("reads a pre-tapToFocus payload without discarding the prefs it does have", () => {
    ...
    const { result } = renderHook(() => useDisplayPrefs());
    expect(result.current.prefs).toEqual({ wrap: false, fontSize: 15, rawTerminal: true, tapToFocus: true, expandClippedReply: true });
  });
