...
export function usePolling(data: HomeData | undefined, paneId?: string | null): void {
  ...
  // Pane-scoped: only the open pane's own last poll feeds rule 3, so switching panes can't leak the
  // previous pane's "still moving" verdict into the new one's cadence. An empty paneId (home
  // screen) reads false, which is correct — rule 3 doesn't apply there anyway.
  const changed = useLastPollChanged(paneId ?? "");
  const sendKick = useSendCount();
  const ms = intervalFor(data, paneId, {
    bursting: burstAppliesTo(burstPane, paneId),
    // The pane view's own follow intent, published to lib/poll-intent, answers — and it is true
    // whenever no pane is open.
