...
export async function checkForUpdate(): Promise<void> {
  ...
  setTimeout(() => {
    ...
    // reloads the page — the same condition `reloadOnce` is about to apply. It has to be written
    // BEFORE `location.reload()`, which does not return.
    if (!reloaded) rememberGuardReload();
    reloadOnce();
  }, STUCK_GUARD_MS);
  try {
    await reg.update();
