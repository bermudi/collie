...
describe("the store", () => {
  it("starts with no burst, following, and nothing known about the last poll", () => {
    ...
    expect(lastPollChanged("w1:p1")).toBe(false); // unknown pane → safe default
  ...
  it("markPollResult drives both the change flag and the burst", () => {
    stampSend("w1:p1");
    markPollResult("w1:p1", true);
    expect(lastPollChanged("w1:p1")).toBe(true);
    expect(burstState().quiet).toBe(0);
    for (let i = 0; i < BURST_MIN_POLLS; i += 1) markPollResult("w1:p1", false);
    expect(lastPollChanged("w1:p1")).toBe(false);
    expect(burstPaneId()).toBeNull(); // the burst spent itself on the quiet polls
  });

  it("scopes the change flag per pane — B's poll does not leak into A's cadence", () => {
    stampSend("w1:p1");
    markPollResult("w1:p1", true);
    expect(lastPollChanged("w1:p1")).toBe(true);
    // Pane B polls while A is mid-output: A's verdict is untouched.
    markPollResult("w1:p2", false);
    expect(lastPollChanged("w1:p1")).toBe(true);
    expect(lastPollChanged("w1:p2")).toBe(false);
  });

  it("does not fold a poll from another pane into the burst's own pane", () => {
    stampSend("w1:p1");
    // A burst on A; B's polls must not count toward A's burst budget.
    for (let i = 0; i < BURST_MIN_POLLS + BURST_QUIET_POLLS; i += 1)
      markPollResult("w1:p2", false);
    expect(burstPaneId()).toBe("w1:p1"); // still armed — B's polls were ignored
    expect(burstState().polls).toBe(0);
  });
