...
import { isLightFill, markLightFills, LIGHT_FILL_LUMA, NEAR_WHITE_FILL_LUMA } from "./light-fill";
...
describe("markLightFills", () => {
  // Identity when nothing matches is a contract the adapters assert on: a screen the agent does not
  // paint this way must come back object-for-object identical.
  it("returns the input array itself when no fill clears the floor", () => {
    const lines: StyledLine[] = [
      { segments: [{ text: "plain", style: {}, muted: false }] },
      { segments: [{ text: "diff", bg: "rgb(33,58,43)", style: {}, muted: false }] },
    ];
    expect(markLightFills(lines, NEAR_WHITE_FILL_LUMA)).toBe(lines);
  });

  it("marks only the segments above the floor, and never marks a segment twice", () => {
    const lines: StyledLine[] = [
      { segments: [{ text: "band", bg: "rgb(244,244,244)", style: {}, muted: false }] },
      { segments: [{ text: "body", bg: "rgb(15,18,22)", style: {}, muted: false }] },
    ];
    const once = markLightFills(lines, NEAR_WHITE_FILL_LUMA);
    expect(once).not.toBe(lines);
    expect(once[0]!.segments[0]!.mobileTransparentBg).toBe(true);
    expect(once[1]!.segments[0]!.mobileTransparentBg).toBeUndefined();

    const twice = markLightFills(once, NEAR_WHITE_FILL_LUMA);
