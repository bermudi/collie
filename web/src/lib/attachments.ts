 ...
 * extension list acceptAttribute builds is what makes a `.md` pickable, and it is also what makes
 * both Android and iOS drop the gallery and open the file browser alone. The two asks cannot
 ...
 * bridge older than the field, through uploadLimits' fallback — has one answer to give, so the
 * button opens the camera roll directly and the operator never sees a choice with one option.
 */
 ...
 * it alone can do. `null` means send it; a string is the reason not to, which the composer turns
 * into a sentence (no i18n in this fork — literals at the call site).
 *
