...
//
// The update surfaces: the settings page carries the check control (the QUESTION) and the build
// stamp, which is a diagnostic and was never an update surface; the actionable lines live in the
// footer's UpdateBanner on the dashboard.
describe("SettingsRoute — the update surfaces", () => {
  it("keeps the build stamp and drops the footer update chip", async () => {
    ...
    // Nothing on this page nudges an update: no chip line, no copyable command.
    expect(screen.queryByText(/Bridge restart needed/)).toBeNull();
    expect(screen.queryByRole("button", { name: /^Copy command/ })).toBeNull();
  });

  it("carries the check control, and no link row to an updates page", async () => {
    renderSettings();
