...
//
// The number itself is per install (`COLLIE_MAX_UPLOAD_MB`), so it is no longer a constant: the
// handler takes it as an argument from cfg.
...
/**
 * Whether a declared `Content-Length` is already too big to be a legal upload. Pure — shared by
 * the pre-check so "too large" means one number everywhere. A missing or unparseable length is
 * NOT oversize here — the handler's own post-parse check is what catches a lying client (and
 * `maxRequestBodySize` catches the rest).
 */
