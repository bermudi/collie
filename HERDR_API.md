...
- **Herdr 0.8.0 Shift+Tab defect:** `shift+tab` is accepted but writes a bare Tab (`0x09`) instead
  of BackTab (`ESC [ Z`). Sending `["Escape","[","Z"]` does not substitute — it leaves visible
  `[Z` in the input. Collie's socket adapter sends the complete raw `ESC [ Z` through
  `pane.send_text`; mixed key queues are split into sequential `send_keys` / `send_text` RPCs so
  their semantic order is preserved.
